...
func (c *command) do_address() error {
   ...
   // Save the proxy state. If -x was used, it saves the value.
   // If -x was not used, it saves an empty string (clearing previous proxy).
   cache.Proxy = c.proxy
...
func (c *command) do_edit() error {
   ...
   // If -x was not provided for this command, reuse the proxy saved
   // during the previous -a command.
   if c.proxy == "" {
      c.proxy = cache.Proxy
   }
