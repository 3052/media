...
// newError takes a variable number of strings, combines them with spaces, and returns an error.
func newError(strs ...string) error {
...
func buildURL(marketCode, endpoint, id string) (string, error) {
   classID, ok := classificationMap[marketCode]
   if !ok {
      // Note: The space is now automatic via newError
      return "", newError("unsupported market code:", marketCode)
   }
   params := url.Values{}
